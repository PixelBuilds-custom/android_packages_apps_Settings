...
public class PropsDownloader extends SettingsPreferenceFragment implements Preference.OnPreferenceChangeListener {
    ...
    public void onCreatePreferences(Bundle savedInstanceState, String rootKey) {
        ...
        Preference playIntegrityCat = findPreference("pintegrity_category");

        getActivity().setTitle(R.string.play_integrity_dashboard_title);
        mContext = getActivity();

        final String[] credsArr = getCredsArr(mContext);
        boolean isAvail = credsArr.length > 0;

        if (playIntegrityCat != null) {
            playIntegrityCat.setVisible(isAvail);
        }

        if (downloadPropertiesPref != null && isAvail) {
            downloadPropertiesPref.setOnPreferenceClickListener(preference -> {
                if (!isDownloading) {
                    if (isInternetAvailable(mContext)){
                        isDownloading = true;

                        dlProps = downloadProps(mContext, credsArr);

                        Log.w(TAG, "PPU arr is " + Arrays.toString(PixelPropsUtils.sCertifiedProps));
                        Log.w(TAG, "dl arr is " + Arrays.toString(dlProps));

                        if (dlProps.length > 0 && PixelPropsUtils.sCertifiedProps.length == 0) {
                            PixelPropsUtils.sCertifiedProps = dlProps;
                            showToast (mContext, "Props are updated!");
                        } else if (dlProps.length > 0 && !Arrays.equals(
                                    PixelPropsUtils.sCertifiedProps, dlProps)) {
                            PixelPropsUtils.sCertifiedProps = dlProps;
                            showToast (mContext, "Props are updated!");
                        } else {
    ...
    private static String[] parseStringArray(String xmlContent) {
        ...
        for (String line : lines) {
            ...
            if (isPropsArray && line.startsWith("<item>") && line.endsWith("</item>")) {
                int start = line.indexOf(">") + 1;
                int end = line.lastIndexOf("<");
                String item = line.substring(start, end).trim();
