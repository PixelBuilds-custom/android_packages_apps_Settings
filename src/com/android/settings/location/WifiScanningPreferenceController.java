...
public class WifiScanningPreferenceController extends AbstractPreferenceController
    ...
    public void updateState(Preference preference) {
        ((SwitchPreferenceCompat) preference).setChecked(mWifiManager.isScanAlwaysAvailable());
    }

    @Override
    public boolean handlePreferenceTreeClick(Preference preference) {
        if (KEY_WIFI_SCAN_ALWAYS_AVAILABLE.equals(preference.getKey())) {
            mWifiManager.setScanAlwaysAvailable(((SwitchPreferenceCompat) preference).isChecked());
            return true;
        }
        return false;
    }
